...
// ─── Showcase: pre-computed result (AI Vendor HIGH, score ~75) ────────────────
const SHOWCASE_RESULT = {
  ...
  executiveSummary: "The AI vendor demonstrates solid foundational compliance with the EU AI Act's high-risk requirements, including documented technical specifications, a GDPR Data Processing Agreement, and reference EU clients. However, formal gaps remain in bias documentation, conformity assessment procedures, and post-market monitoring. These are remediable within the August 2026 deadline with focused effort. The vendor is well-positioned to achieve full compliance with targeted investments in documentation and governance.",
  redFlags: [
  ...
  gaps: [
    {
      ...
      gapLevel: "HIGH",
      gapTitle: "Risk Management System Incomplete",
      gapDescription: "While the vendor has basic QMS documentation, there is no evidence of a continuous risk management system specifically addressing AI-related risks: bias propagation, data drift, adversarial inputs, and disparate impact across candidate demographics.",
      actions: [
        { priority: "HIGH", action: "Establish a documented AI risk register with identified risks, likelihood, impact, and mitigation measures", deadline: "BEFORE_2026-08-02" },
        { priority: "MEDIUM", action: "Implement automated monitoring to detect model drift and trigger re-evaluation cycles", deadline: "ONGOING" },
      ],
    ...
    },
    {
      ...
      gapLevel: "HIGH",
      gapTitle: "Training Data Governance Gaps",
      gapDescription: "No documentation provided on training dataset composition, demographic representativeness, or data quality measures. For a system scoring candidates, the absence of dataset transparency is a critical compliance gap that also constitutes a significant legal liability.",
      actions: [
        ...
        { priority: "HIGH", action: "Implement data quality processes including bias detection at the data pipeline level", deadline: "BEFORE_2026-08-02" },
      ],
      estimatedEffort: "HIGH",
    },
    {
      ...
      gapLevel: "MEDIUM",
      gapTitle: "Transparency to Deployers Partial",
      gapDescription: "Technical documentation exists but instructions for use lack sufficient detail on system limitations, known biases, and conditions under which the system may underperform. Deployers (HR teams) need clearer guidance to exercise meaningful oversight.",
      actions: [
        { priority: "MEDIUM", action: "Expand instructions for use to include known limitations, edge cases, and recommended human review triggers", deadline: "BEFORE_2026-08-02" },
      ],
      estimatedEffort: "LOW",
    },
    {
      ...
      gapLevel: "MEDIUM",
      gapTitle: "Human Oversight Controls Need Strengthening",
      gapDescription: "The system provides scores and rankings but the override mechanism for human reviewers is not formally documented. There is no logged audit trail of cases where human judgment overrode the AI recommendation, which is required for accountability.",
      actions: [
        { priority: "MEDIUM", action: "Implement and document human override workflow with mandatory logging of override decisions and rationale", deadline: "BEFORE_2026-08-02" },
      ],
      estimatedEffort: "MEDIUM",
    },
    {
      articleId: "art43",
      articleNumber: "Article 43",
      gapLevel: "HIGH",
      gapTitle: "Conformity Assessment Not Initiated",
      gapDescription: "No conformity assessment procedure has been initiated. For high-risk AI systems in employment, this is a mandatory pre-market requirement. Without it, the system cannot legally be placed on the EU market.",
      actions: [
        { priority: "HIGH", action: "Initiate conformity assessment — determine self-assessment eligibility or identify applicable notified body", deadline: "BEFORE_2026-08-02" },
      ],
      estimatedEffort: "HIGH",
    },
    {
      ...
      gapLevel: "LOW",
      gapTitle: "Quality Management System Partially Documented",
      gapDescription: "Basic QMS exists covering standard software development practices. AI-specific quality processes (model versioning, retraining governance, performance benchmarks) are not yet formalized.",
      actions: [
        { priority: "LOW", action: "Extend QMS to cover AI-specific lifecycle: model versioning policy, retraining triggers, performance threshold definitions", deadline: "BEFORE_2026-08-02" },
      ],
      estimatedEffort: "MEDIUM",
    },
    {
      ...
      gapLevel: "LOW",
      gapTitle: "Post-Market Monitoring Plan Missing",
      gapDescription: "No post-market monitoring plan documented. For high-risk AI, providers must proactively collect and review performance data from deployed systems to detect issues including emergent bias.",
      actions: [
        { priority: "MEDIUM", action: "Draft and implement a post-market monitoring plan with KPIs, review cadence, and incident reporting procedure", deadline: "ONGOING" },
      ],
      estimatedEffort: "MEDIUM",
    },
  ],
...
const s = {
  ...
  keyInput: {
    ...
    padding: "1rem", fontFamily: "'DM Mono', monospace",
    outline: "none", boxShadow: "0 1px 3px rgba(0,0,0,0.05)",
    marginBottom: "1rem",
  },
  checkboxLabel: {
    display: "flex", alignItems: "center", gap: 8,
    fontSize: 12, color: C.textSub, marginBottom: "1.5rem",
      ...
      >
        <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: '1.5rem' }}>
          <div>
            ...
            <div style={{ fontFamily: "'Playfair Display', Georgia, serif", fontSize: 22, fontWeight: 700, color: C.text }}>
        ...
        <div style={{ borderTop: `1px solid ${C.border}`, paddingTop: '1.25rem', marginTop: '0.5rem' }}>
          ...
          <div style={{ fontSize: 12, color: C.textSub, lineHeight: 1.75 }}>
            Each gap analysis uses the <strong style={{ color: C.text }}>claude-sonnet-4-20250514</strong> model and costs approximately <strong style={{ color: C.text }}>$0.02 – $0.05</strong> depending on the length of your system description and the number of articles analysed. Running 20 analyses costs less than $1.
          </div>
        </div>
        ...
        <div style={{ marginTop: '1.25rem', display: 'flex', justifyContent: 'flex-end' }}>
          <button
            style={{
              fontSize: 11, letterSpacing: '0.1em', textTransform: 'uppercase',
              padding: '10px 20px', background: C.goldFaint,
...
export default function App() {
  ...
  async function runAnalysis() {
    if (!apiKey) return; // safety check: gate normally prevents this
    ...
    try {
      ...
      const data = await runGapAnalysis(profile, description, articleIds, apiKey);
      setResult(data);
      setPhase("result");
    } catch (e) {
      setError(e.message || "Analysis error. Please check your description and try again.");
      setPhase("input");
    }
  }
    ...
    <div style={s.root}>
      ...
      <header style={s.header}>
        ...
        <div style={s.headerRight}>
          {apiKey && (
            <button style={s.changeKeyBtn} onClick={handleClearKey}>
              Change Key
            </button>
          )}
          ...
          <>
            <div style={s.hero}>
              ...
              <h1 style={s.heroTitle}>
                Assess your AI system's<br />
                <span style={{ color: C.gold }}>EU AI Act compliance</span>
              </h1>
              <p style={s.heroSub}>
                Describe your AI system and get an instant gap analysis: risk classification,
                compliance score, article-by-article gaps, and a prioritised remediation plan —
                all generated against the live EU AI Act framework.
              </p>
            ...
            <input
              type="password"
              value={keyInput}
              onChange={(e) => { setKeyInput(e.target.value); if (keyError) setKeyError(""); }}
            ...
            <button
              style={s.analyzeBtn(!keyInput.trim())}
            ...
            <div style={{ fontSize: 11, color: C.textMuted, marginTop: 8, lineHeight: 1.6 }}>
              Preview a pre-computed analysis — no API key required.
            </div>
            ...
            <div style={s.privacyNote}>
              ...
              <div style={{ marginTop: 8 }}>
                ...
                • Recommended: create a dedicated key for this tool so you can revoke it anytime without impacting other projects.
              </div>
            </div>
          </>
        )}
          ...
          <>
            {!apiKey && (
              <div style={{
                ...
                <span style={{ fontSize: 11, color: C.gold, letterSpacing: "0.08em", textTransform: "uppercase" }}>
                  // Showcase — pre-computed example scenario
                </span>
                <button
                  style={{ ...s.changeKeyBtn, borderColor: C.gold, color: C.gold }}
                  onClick={() => { setPhase("input"); setResult(null); setProfile(null); }}
                >
...
// ─── API Call ────────────────────────────────────────────────────────────────
async function runGapAnalysis(profile, description, articleIds, apiKey) {
...
`).join("\n---\n")}`;
  ...
  if (response.status === 401) {
    throw new Error("Invalid or expired API key. Verify the value and credit availability.");
  }
  if (response.status === 429) {
    throw new Error("Anthropic rate limit hit. Retry in a few minutes.");
  }
  if (!response.ok) {
